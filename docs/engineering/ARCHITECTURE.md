...
```text
Browser: upload / review / selection / M6 preview / M7 edit + delete / M8 columns / M9 TSV download
...
| Export-column configuration  | Store one shared allowlisted FE/BE selection using OQ-07 defaults and fixed schema order                                                                                                    | No reorder controls, arbitrary keys, case mutation, or AI request                                                                                          |
| TSV generation               | Serialize current saved edits with selected columns in fixed schema order and download separate nonempty FE/BE files                                                                        | Deterministic browser code; no AI/server request, combined file, internal metadata, or state mutation                                                      |
...
## Implemented M8 configuration boundary

M8 stores only unique allowlisted column keys. The first nine schema fields default selected; Automation and Notes default unselected. Selection transitions and strict validation preserve canonical schema order, so no reorder control or alternate order state exists. One preference is shared by FE/BE and retained independently from generation and M7 mutations in active-session storage. Invalid persisted configuration falls back safely. M8 adds no TSV serialization, export route, Blob, download, filename, or provider activity.

## Implemented M9 export boundary

M9 reads the current M7 result and validated M8 selection, maps only approved public fields, preserves row/ID order, flattens numbered Steps with `|`, normalizes embedded TAB/CR/LF, and prefixes formula-leading values with an apostrophe in the export representation. The download layer adds a UTF-8 BOM and uses CRLF/TAB plus fixed layer filenames. Empty or invalid layers do not download. Temporary object URLs are revoked. No API, provider call, result mutation, combined file, history, XLSX, or persistence is added. See [M9 TSV Export](M9_TSV_EXPORT.md).

## Implementation entry conditions

Before implementing later milestones, inspect state and select compatible tools within the approved direction. M3 establishes one server-configurable Gemini adapter and defaults it to the live-validated `gemini-3.5-flash`; no provider selector. M4 creates an identity-bound reviewed-selection contract. M5 consumes it through selected-only FE/BE generation, strict reference validation and application-owned IDs. M6 renders that result, M7 mutates its working copy, M8 stores the fixed-order selected-column preference, and M9 exports their current projection locally. AI calls use analysis, frontend-generation and backend-generation usage actions with missing counters null. No unresolved MVP-blocking Open Questions remain.
