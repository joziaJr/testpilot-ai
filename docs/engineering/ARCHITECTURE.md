...
Approved direction: one full-stack web application using preferred Next.js, TypeScript, Tailwind CSS, shadcn/ui, and Zod. Use a simple table or justify TanStack Table; no microservices or unnecessary infrastructure. [PRD](../product/PRD.md), [Business Flow](../product/BUSINESS_FLOW.md), and [resolved OQ-03–OQ-14](../product/OPEN_QUESTIONS.md) govern the design. M0 is installed; M1 implements [upload validation](M1_UPLOAD.md), M2 implements [document extraction](M2_EXTRACTION.md), M3 implements the [AI PRD Analyzer](M3_AI_PRD_ANALYZER.md), M4 implements [requirement review and selection](M4_REQUIREMENT_REVIEW.md), and M5 implements [AI test-case generation](M5_TEST_CASE_GENERATION.md). Full preview/edit/delete and export remain planned.
...
| Test Case Generator          | Generate strict FE/BE Positive/Negative/Edge drafts from only the reviewed selection, validate grounding, deduplicate, and assign deterministic IDs                                         | No unselected expansion or invented API/product contracts; minimal M5 visibility is not the final editable preview                                         |
...
Before implementing later milestones, inspect state and select compatible tools within the approved direction. M3 establishes one server-configurable Gemini adapter and defaults it to the live-validated `gemini-3.5-flash`; no provider selector. M4 creates an identity-bound reviewed-selection contract. M5 validates and consumes that contract with separate `generate_frontend` and `generate_backend` actions and null missing token counters. No unresolved MVP-blocking Open Questions remain. M6 preview requires separate authorization.
