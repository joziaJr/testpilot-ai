...
Approved direction: one full-stack web application using preferred Next.js, TypeScript, Tailwind CSS, shadcn/ui, and Zod. Use a simple table or justify TanStack Table; no microservices or unnecessary infrastructure. [PRD](../product/PRD.md), [Business Flow](../product/BUSINESS_FLOW.md), and [resolved OQ-03–OQ-14](../product/OPEN_QUESTIONS.md) govern the design. M0 is installed; M1 implements [upload validation](M1_UPLOAD.md), M2 implements server-side [document extraction](M2_EXTRACTION.md), and M3 implements the server-side [AI PRD Analyzer](M3_AI_PRD_ANALYZER.md). Review, generation and export responsibilities remain planned.
...
Before implementing later milestones, inspect state and select compatible tools within the approved direction. M3 establishes one server-configurable Gemini adapter and defaults it to the live-validated `gemini-3.5-flash`; no provider selector. Both will use analysis, frontend-generation and backend-generation usage actions with missing counters null. No unresolved MVP-blocking Open Questions remain. M4 requirement review requires separate authorization.
