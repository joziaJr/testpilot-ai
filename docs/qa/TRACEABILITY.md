...
PRD sections 21–23/27/29/33–35, Business Flow Select Export Columns, and OQ-06/OQ-07/OQ-13 map the M7 working result to one strict `selectedColumns` preference. [M8 FE specifications](../../qa/test-cases/frontend/M8_EXPORT_COLUMN_SELECTION.md), [backend-boundary specifications](../../qa/test-cases/backend/M8_EXPORT_COLUMN_SELECTION.md), contract/component tests, and browser tests cover first-nine defaults, the eleven-key allowlist, canonical order, session restoration/fallback, FE/BE sharing, M7 isolation, accessibility, and network/download absence. [M8 Export Column Selection](../engineering/M8_EXPORT_COLUMN_SELECTION.md) defines the exact boundary. TSV generation and files remain M9 and are not claimed.

## M9 implementation

PRD sections 21–23/27/33–35, Business Flow Export TSV, and OQ-07/OQ-08/OQ-09/OQ-13 map current M7 working cases plus validated M8 `selectedColumns` to the deterministic serializer and separate layer downloads. [M9 FE specifications](../../qa/test-cases/frontend/M9_TSV_EXPORT.md), serializer/component tests, and browser artifact assertions cover selected headers, FE/BE filenames and separation, current edits/deletes and ID gaps, OQ-08 Steps/normalization/BOM/CRLF/TAB, formula-safe literals, local-only behavior, empty/invalid prevention, and URL cleanup. [M9 TSV Export](../engineering/M9_TSV_EXPORT.md) defines the boundary. No backend route, XLSX, history, execution record, or bug evidence is claimed.
